...
class DummyModelForTest(models.Model):
    name = models.CharField(u"Dummy Name", max_length=150)
...
def utils_create_two_badges(test_case):
    ...
    test_case.badge1 = Badge.objects.create(name=u"badge1", name_is_visible=True,
                                            condition_is_visible=True, is_active=True, content_type=ct,
                                            simple_condition=NUMBER_RELATED_MODEL, name_field=u'user',
                                            value_for_valid=1)
    test_case.badge2 = Badge.objects.create(name=u"badge2", name_is_visible=True,
                                            condition_is_visible=True, is_active=True, content_type=ct,
                                            simple_condition=NUMBER_RELATED_MODEL, name_field=u'user',
                                            value_for_valid=3)
